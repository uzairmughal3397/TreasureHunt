...
import com.daniyal.treasurehunting.databinding.FragmentSearchMapsBinding
import com.daniyal.treasurehunting.helpers.AppConstants
import com.daniyal.treasurehunting.helpers.SharedPrefHelper
...
class SearchMapsFragment : Fragment(), LocationListener {
    ...
    private fun setUpMarkersFromDb(googleMap: GoogleMap) {

        /*Setting up the radius*/
        val radius = if (SharedPrefHelper().readString(requireContext(), AppConstants.RADIUS_KEY)
                .isNullOrEmpty()
        ) {
            25
        } else
